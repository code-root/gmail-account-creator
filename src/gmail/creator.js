...
const GMAIL_URL = 'https://accounts.google.com/signup/v2/createaccount?flowName=GlifWebSignIn&flowEntry=SignUp';
...
export async function createGmailAccount(page, accountData) {
    ...
    try {
        ...
        // تأخير بشري قبل التفاعل
        await humanSleep(getHumanDelay('thinking'));
        // Variable delay - simulate reading dropdown options
        // تأخير متغير - محاكاة قراءة خيارات القائمة
        await humanSleep(getHumanDelay('reading'));
        // Human behavior: Variable delay and page exploration
        // سلوك بشري: تأخير متغير واستكشاف الصفحة
        await humanSleep(getHumanDelay('pageLoad'));
        await simulateReading(page, { duration: 1500 + Math.random() * 2000, scrollWhileReading: true });
        ...
        // Human behavior: Variable delay after form submission
        // سلوك بشري: تأخير متغير بعد إرسال النموذج
        ...
        if (hasBirthdayFields) {
            ...
            birthDate = await fillBirthdayAndGenderForm(page, accountData);
            // Human behavior: Variable delay after form submission
            await humanSleep(getHumanDelay('pageLoad'));
            await performRandomAction(page); // Random action while waiting
        }
        ...
        if (pageAfterName) {
            ...
            if (pageAfterName.type === 'BIRTHDAY_GENDER') {
                ...
                // Human behavior: Variable delay with random actions
                await humanSleep(getHumanDelay('pageLoad'));
            ...
            if ((currentUrl.includes('/password') || 
                ...
                try {
                    ...
                    if (passwordFields.length > 0) {
                        ...
                        // Click Next button
                        await humanSleep(getHumanDelay('shortPause'));
...
async function fillBirthdayAndGenderForm(page, accountData) {
    try {
        ...
        // ملء الشهر (قائمة منسدلة مخصصة - النقر لفتحها، ثم اختيار الخيار)
        let monthFilled = false;
        const monthNames = ['January', 'February', 'March', 'April', 'May', 'June',
            'July', 'August', 'September', 'October', 'November', 'December'];
        const monthName = monthNames[birthMonth - 1];

        // Method 1: Try using jsname selector (most reliable for Material Design)
        // الطريقة 1: محاولة استخدام jsname selector (الأكثر موثوقية لـ Material Design)
        try {
            const monthDropdown = await waitForElement(page, 'div[jsname="O1htCb"]', 3000);
            if (monthDropdown) {
                // Human behavior: Scroll to element and move mouse
                await humanScrollToElement(page, monthDropdown);
                await humanSleep(getHumanDelay('beforeClick'));
                
                // Click to open dropdown
                await humanClick(page, monthDropdown, { preClickDelay: true, postClickDelay: false });
                logger.info('Month dropdown opened (jsname method)');
                
                // Wait for dropdown menu to appear
                await humanSleep(getHumanDelay('shortPause'));
                
                // Try to find and click the month option by data-value
                ...
                if (monthOptions.length > 0) {
                        ...
                        if (isVisible) {
                            await humanClick(page, monthOptions[0], { preClickDelay: true, postClickDelay: true });
                            logger.info('Month selected by data-value', { month: birthMonth, monthName });
                    monthFilled = true;
                        }
                    }
                } catch (e) {
                ...
                // Fallback: Find by text if data-value failed
                if (!monthFilled) {
                    ...
                    if (monthTextOptions.length > 0) {
                            ...
                            if (isVisible) {
                                await humanClick(page, monthTextOptions[0], { preClickDelay: true, postClickDelay: true });
                        logger.info('Month selected by text', { month: monthName });
                        monthFilled = true;
                    }
                }
                    } catch (e) {
                        logger.debug('Text search failed', { error: e.message });
                    }
                }
                
                // Fallback: Use JavaScript to click directly
                if (!monthFilled) {
                    try {
                        await page.evaluate((monthValue, monthText) => {
                            // Find all month options
                            ...
                            const targetOption = options.find(opt => {
                                ...
                                return value === monthValue || text.includes(monthText);
                            });
                        ...
                        logger.info('Month selected by JavaScript', { month: monthName });
                        monthFilled = true;
        } catch (e) {
                        logger.debug('JavaScript method failed', { error: e.message });
                    }
                }
            }
        } catch (e) {
            logger.debug('jsname method failed, trying other methods', { error: e.message });
        }

        // Method 2: Try using ID selector
        // الطريقة 2: محاولة استخدام ID selector
        if (!monthFilled) {
            try {
                const monthDropdown = await waitForElement(page, '#month', 3000);
                if (monthDropdown) {
                    await humanScrollToElement(page, monthDropdown);
                    await humanSleep(getHumanDelay('beforeClick'));
                    await humanClick(page, monthDropdown, { preClickDelay: true, postClickDelay: false });
                    logger.info('Month dropdown opened (ID method)');
                    await humanSleep(getHumanDelay('shortPause'));
                    ...
                    try {
                        ...
                        if (monthOptions.length > 0) {
                            await humanClick(page, monthOptions[0], { preClickDelay: true, postClickDelay: true });
                            logger.info('Month selected (ID method)', { month: monthName });
                            monthFilled = true;
                        }
                    } catch (e) {
                        // Try JavaScript fallback
                        await page.evaluate((monthValue) => {
                            const option = document.querySelector(`li[role="option"][data-value="${monthValue}"]`);
                ...
                logger.debug('ID method failed', { error: e.message });
            }
        }

        // Method 3: Try using XPath (original method as fallback)
        // الطريقة 3: محاولة استخدام XPath (الطريقة الأصلية كبديل)
        if (!monthFilled) {
            try {
                ...
                if (monthDropdowns.length > 0) {
                    await humanClick(page, monthDropdowns[0], { preClickDelay: true, postClickDelay: false });
                    logger.info('Month dropdown opened (XPath method)');
                    await humanSleep(getHumanDelay('shortPause'));
                    ...
                    if (monthOptions.length > 0) {
                        await humanClick(page, monthOptions[0], { preClickDelay: true, postClickDelay: true });
                        logger.info('Month selected (XPath method)', { month: monthName });
                        monthFilled = true;
                    }
                }
            } catch (e) {
                logger.debug('XPath method failed', { error: e.message });
            }
        }

        // Method 4: Try traditional select element (last resort)
        // الطريقة 4: محاولة استخدام عنصر select التقليدي (آخر محاولة)
        if (!monthFilled) {
            ...
            for (const selector of monthSelectors) {
                try {
                    ...
                    if (select) {
                        ...
                        logger.info('Month selected by select element', { month: monthName });
                        monthFilled = true;
                        break;
                    }
                } catch (e) {
            ...
            try {
                ...
                if (isTimeout && lastPhoneOrder && lastPhoneOrder.id) {
                    try {
                        ...
                        await humanSleep(getHumanDelay('shortPause')); // Small delay
...
async function completeAccountCreation(page, accountData) {
    try {
        ...
        // Step 1: Handle Recovery Email page - SKIP (don't enter recovery email)
        // الخطوة 1: التعامل مع صفحة البريد الإلكتروني للاسترداد - تخطي (عدم إدخال إيميل استرداد)
        if (currentUrl.includes('addrecoveryemail')) {
            logger.info('Recovery email page detected - skipping recovery email entry');
            
            // Human behavior: Wait a bit before skipping
            // سلوك بشري: الانتظار قليلاً قبل التخطي
            await humanSleep(getHumanDelay('thinking'));
            
            // Try to find and click "Skip" button first
            // محاولة العثور على زر "Skip" والنقر عليه أولاً
            let skipClicked = false;
            
            // Method 1: Look for "Skip" button by text
            // الطريقة 1: البحث عن زر "Skip" بالنص
            try {
                ...
                if (skipButton && skipButton.asElement()) {
                    ...
                    logger.info('Skip button clicked on recovery email page');
                    skipClicked = true;
                    await humanSleep(getHumanDelay('pageLoad'));
                }
            } catch (e) {
            ...
            // Method 2: Look for "Skip" link by aria-label or href
            // الطريقة 2: البحث عن رابط "Skip" بـ aria-label أو href
            if (!skipClicked) {
                try {
                    const skipSelectors = [
                        'a[aria-label*="skip" i]',
                    ...
                    for (const selector of skipSelectors) {
                        try {
                            const skipElement = await page.$(selector);
                            if (skipElement) {
                                await humanClick(page, skipElement, { preClickDelay: true, postClickDelay: true });
                                logger.info('Skip element clicked', { selector });
                                skipClicked = true;
                                await humanSleep(getHumanDelay('pageLoad'));
                        break;
                    }
                    ...
                    logger.debug('Could not find Skip element by selectors', { error: e.message });
                }
            }
            
            // Method 3: If no Skip button found, try clicking Next without entering email
            // الطريقة 3: إذا لم يتم العثور على زر Skip، جرب النقر على Next بدون إدخال إيميل
            if (!skipClicked) {
                logger.info('No Skip button found, trying to click Next without entering email');
                
                // Wait a bit (simulating human reading the page)
                await humanSleep(getHumanDelay('reading'));
            ...
            try {
                ...
                const nextButtons = await page.$x(nextButtonXPath);
                if (nextButtons.length > 0) {
                        const isVisible = await nextButtons[0].evaluate(el => el.offsetParent !== null);
                        const isDisabled = await nextButtons[0].evaluate(el => el.disabled);
                        
                        if (isVisible && !isDisabled) {
                            await humanClick(page, nextButtons[0], { preClickDelay: true, postClickDelay: true });
                            logger.info('Next button clicked on recovery email page (skipping email entry)');
                            skipClicked = true;
                            await humanSleep(getHumanDelay('pageLoad'));
                        }
                }
                ...
                // Fallback: Use clickNextButton function
                if (!skipClicked) {
                    const nextClicked = await clickNextButton(page);
                    if (nextClicked) {
                        logger.info('Next button clicked via fallback method (skipping email entry)');
                        await humanSleep(getHumanDelay('pageLoad'));
                    }
                }
            }
            
            // Wait for navigation after skipping
            await humanSleep(getHumanDelay('pageLoad'));
            currentUrl = page.url();
            logger.info('Recovery email page skipped successfully', { newUrl: currentUrl });
        }
        ...
        if (currentUrl.includes('termsofservice')) {
            ...
            // After agreeing to terms, check if confirmidentifier page appears
            // بعد الموافقة على الشروط، تحقق من ظهور صفحة confirmidentifier
            await humanSleep(getHumanDelay('pageLoad'));
            try {
                currentUrl = page.url();
                if (currentUrl.includes('confirmidentifier')) {
                    logger.info('Confirm identifier page detected after terms agreement');
                    // Will be handled in Step 4 below
                    // سيتم التعامل معها في الخطوة 4 أدناه
                }
            } catch (urlError) {
                logger.debug('Could not get URL after terms agreement', { error: urlError.message });
            }
        }

        // Step 4: Handle Recovery Options page - Click Cancel
        // الخطوة 4: التعامل مع صفحة خيارات الاسترداد - الضغط على Cancel
        if (currentUrl.includes('recoveryoptions') || currentUrl.includes('/web/recoveryoptions')) {
            logger.info('Recovery options page detected - clicking Cancel button');
            
            await humanSleep(getHumanDelay('thinking'));
            
            // Try to find and click Cancel button
            // محاولة العثور على زر Cancel والنقر عليه
            let cancelClicked = false;
            
            // Method 1: Try by aria-label
            try {
                const cancelButton = await page.$('button[aria-label="Cancel"], button[jsname="ZUkOIc"]');
                if (cancelButton) {
                    const isVisible = await cancelButton.evaluate(el => el.offsetParent !== null);
                    const isDisabled = await cancelButton.evaluate(el => el.disabled);
                    if (isVisible && !isDisabled) {
                        await humanClick(page, cancelButton, { preClickDelay: true, postClickDelay: true });
                        logger.info('Cancel button clicked on recovery options page');
                        cancelClicked = true;
                        await humanSleep(getHumanDelay('pageLoad'));
                    }
                }
            } catch (e) {
                logger.debug('Could not find Cancel button by aria-label', { error: e.message });
            }
            ...
            if (!cancelClicked) {
                try {
                    ...
                    if (cancelButton && cancelButton.asElement()) {
                        await humanClick(page, cancelButton.asElement(), { preClickDelay: true, postClickDelay: true });
                        logger.info('Cancel button clicked by text search');
                        cancelClicked = true;
                        await humanSleep(getHumanDelay('pageLoad'));
                    }
                } catch (e) {
                    logger.debug('Could not find Cancel button by text', { error: e.message });
                }
            }
            ...
            if (cancelClicked) {
                ...
                try {
                    ...
                    logger.debug('Could not get URL after Cancel click');
                }
            }
        }

        // Step 4.5: Handle Password page (after recovery options or at any point)
        // الخطوة 4.5: التعامل مع صفحة كلمة المرور (بعد خيارات الاسترداد أو في أي وقت)
        if (currentUrl.includes('/challenge/pwd') || currentUrl.includes('signin/challenge/pwd')) {
            logger.info('Password challenge page detected - entering password');
            
            await humanSleep(getHumanDelay('thinking'));
            
            // Enter password using the specified XPath
            // إدخال كلمة المرور باستخدام XPath المحدد
            ...
            try {
                ...
                const passwordInputs = await page.$x(passwordInputXPath);
                if (passwordInputs.length > 0) {
                    const passwordInput = passwordInputs[0];
                    
                    // Click to focus
                    await humanClick(page, passwordInput, { preClickDelay: true, postClickDelay: false });
                    await humanSleep(getHumanDelay('typing'));
                    
                    // Clear any existing text and type password
                    await passwordInput.click({ clickCount: 3 });
                    await page.waitForTimeout(200);
                    
                    await humanType(page, passwordInput, accountData.password, {
                        typingSpeed: 'normal',
                    ...
                    logger.info('Password entered on challenge page (XPath)');
                    passwordEntered = true;
                    await humanSleep(getHumanDelay('afterTyping'));
                }
            } catch (e) {
                logger.debug('XPath method failed, trying CSS selectors', { error: e.message });
            }
            
            // Method 2: Fallback to CSS selectors
            if (!passwordEntered) {
                    ...
                    try {
                        ...
                        if (passwordInput) {
                            ...
                            if (isVisible) {
                                ...
                                logger.info('Password entered on challenge page (CSS)', { selector });
                                passwordEntered = true;
                                await humanSleep(getHumanDelay('afterTyping'));
                                break;
                            }
            ...
            // الضغط على زر Next بعد إدخال كلمة المرور
            if (passwordEntered) {
                await humanSleep(getHumanDelay('beforeClick'));
                
                let nextClicked = false;
                
                // Method 1: Try XPath for Next button
                const nextButtonXPath = '/html/body/div[2]/div[1]/div[1]/div[2]/c-wiz/main/div[3]/div/div/div/div/button';
                try {
                    await page.waitForXPath(nextButtonXPath, { timeout: 5000 });
                    const nextButtons = await page.$x(nextButtonXPath);
                    if (nextButtons.length > 0) {
                        ...
                        if (isVisible && !isDisabled) {
                            await humanClick(page, nextButtons[0], { preClickDelay: true, postClickDelay: true });
                            logger.info('Next button clicked after password entry');
                            nextClicked = true;
                            await humanSleep(getHumanDelay('pageLoad'));
                        }
                    }
                ...
                // Method 2: Try clickNextButton function
                if (!nextClicked) {
                    try {
                        const clicked = await clickNextButton(page);
                        if (clicked) {
                            logger.info('Next button clicked via clickNextButton function');
                            nextClicked = true;
                            await humanSleep(getHumanDelay('pageLoad'));
                        }
                    } catch (e) {
                        logger.debug('clickNextButton method failed', { error: e.message });
                    }
                }
                ...
                if (!nextClicked) {
                    try {
                        const nextButton = await page.evaluateHandle(() => {
                            ...
                            return buttons.find(btn => {
                                const text = (btn.textContent || '').toLowerCase().trim();
                                return (text === 'next' || text === 'التالي' || text === 'continue' || text === 'متابعة') && 
                                       !btn.disabled && 
                                       btn.offsetParent !== null;
                            });
                        });
                        ...
                        if (nextButton && nextButton.asElement()) {
                            ...
                            logger.info('Next button clicked by text search');
                            nextClicked = true;
                            await humanSleep(getHumanDelay('pageLoad'));
                        }
                    } catch (e) {
                ...
                // Get new URL after clicking Next
                if (nextClicked) {
                    await humanSleep(getHumanDelay('pageLoad'));
                    try {
                        currentUrl = page.url();
                        logger.info('Navigation completed after password entry', { newUrl: currentUrl });
                    } catch (e) {
                        logger.debug('Could not get URL after password entry', { error: e.message });
                    }
                } else {
                    logger.warn('Could not click Next button after password entry, but continuing...');
                }
            } else {
                logger.warn('Could not enter password on challenge page, but continuing...');
            }
        }

        // Step 4.6: Handle Consent page (after password or at any point)
        // الخطوة 4.6: التعامل مع صفحة الموافقة (بعد كلمة المرور أو في أي وقت)
        if (currentUrl.includes('/challenge/ipp/consent') || currentUrl.includes('ipp/consent')) {
            logger.info('Consent page detected - clicking Allow/Continue button');
            
            await humanSleep(getHumanDelay('thinking'));
            
            // Click consent button using the specified XPath
            // الضغط على زر الموافقة باستخدام XPath المحدد
            ...
            try {
                ...
                const consentButtons = await page.$x(consentButtonXPath);
                if (consentButtons.length > 0) {
                    const isVisible = await consentButtons[0].evaluate(el => el.offsetParent !== null);
                    const isDisabled = await consentButtons[0].evaluate(el => el.disabled);
                    if (isVisible && !isDisabled) {
                        await humanClick(page, consentButtons[0], { preClickDelay: true, postClickDelay: true });
                        logger.info('Consent button clicked on consent page (XPath)');
                        consentClicked = true;
                        await humanSleep(getHumanDelay('pageLoad'));
                    }
                }
            ...
            if (!consentClicked) {
                try {
                    ...
                    if (consentButton && consentButton.asElement()) {
                        await humanClick(page, consentButton.asElement(), { preClickDelay: true, postClickDelay: true });
                        logger.info('Consent button clicked by text search');
                        consentClicked = true;
                        await humanSleep(getHumanDelay('pageLoad'));
                    }
                } catch (e) {
            ...
            if (!consentClicked) {
                try {
                    const consentSelectors = [
                        ...
                        'button[aria-label*="موافق" i]',
                        'button[aria-label*="Continue" i]',
                        'button[aria-label*="متابعة" i]',
                        ...
                        try {
                            ...
                            if (button) {
                                ...
                                if (isVisible && !isDisabled) {
                                    ...
                                    logger.info('Consent button clicked via CSS selector', { selector });
                                    consentClicked = true;
                                    await humanSleep(getHumanDelay('pageLoad'));
                                    break;
                                }
                    ...
                    logger.debug('CSS selector method failed', { error: cssError.message });
                }
            }
            ...
            if (consentClicked) {
                ...
                try {
                    currentUrl = page.url();
                    logger.info('Navigation completed after consent', { newUrl: currentUrl });
                } catch (e) {
                    logger.debug('Could not get URL after consent click', { error: e.message });
                }
            } else {
                logger.warn('Could not click consent button, but continuing...');
            }
        }

        // Step 5: Handle Home Address page
        // الخطوة 5: التعامل مع صفحة العنوان المنزلي
        if (currentUrl.includes('homeaddress') || currentUrl.includes('/web/homeaddress')) {
            logger.info('Home address page detected');
            
            await humanSleep(getHumanDelay('thinking'));
            
            // Randomly decide whether to enter address (50% chance)
            // قرار عشوائي حول ما إذا كان سيتم إدخال العنوان (50% احتمال)
            let shouldEnterAddress = Math.random() < 0.5;
            let addressEntered = false;
            
            if (shouldEnterAddress) {
                logger.info('Will enter address on home address page');
                
                // Load addresses from config
                // تحميل العناوين من الإعدادات
                let addresses = [];
                try {
                    const __filename = fileURLToPath(import.meta.url);
                    const __dirname = dirname(__filename);
                    const addressesPath = join(__dirname, '../../config/addresses.json');
                    const addressesData = JSON.parse(readFileSync(addressesPath, 'utf8'));
                    const allAddresses = [
                        ...(addressesData.saudi_arabia || []),
                        ...(addressesData.egypt || []),
                        ...(addressesData.uae || [])
                    ];
                    addresses = allAddresses;
                } catch (e) {
                    logger.warn('Could not load addresses from config, using default', { error: e.message });
                    // Default addresses if file not found
                    addresses = [
                        'شارع الملك فهد، حي العليا، الرياض 12211',
                        'شارع النيل، حي الزمالك، القاهرة 11211',
                        'شارع الشيخ زايد، دبي مارينا، دبي'
                    ];
                }
                ...
                // Enter address in input field
                // إدخال العنوان في حقل الإدخال
                try {
                    const addressInputXPath = '/html/body/div[1]/c-wiz[3]/div/div/div/div/div/div[1]/div[2]/div/div[1]/div/div[1]/label/input';
                    await page.waitForXPath(addressInputXPath, { timeout: 5000 });
                    const addressInputs = await page.$x(addressInputXPath);
                    
                    if (addressInputs.length > 0) {
                        await addressInputs[0].click();
                        await humanSleep(getHumanDelay('typing'));
                        await humanType(page, addressInputs[0], randomAddress, { 
                            typingSpeed: 'normal',
                            pauseBetweenWords: true 
                        });
                        logger.info('Address entered on home address page', { address: randomAddress });
                        await humanSleep(getHumanDelay('afterTyping'));
                        addressEntered = true;
                        ...
                        // الضغط على زر Save
                        try {
                            const saveButton = await page.$('button[aria-label="Save"], button[jsname="cSZ6pd"]');
                            if (saveButton) {
                                ...
                                if (isVisible && !isDisabled) {
                                    await humanSleep(getHumanDelay('beforeClick'));
                                    await humanClick(page, saveButton, { preClickDelay: true, postClickDelay: true });
                                    logger.info('Save button clicked after entering address');
                                    await humanSleep(getHumanDelay('pageLoad'));
                                }
                            }
                        } catch (e) {
                            logger.debug('Could not click Save button', { error: e.message });
                        }
                    }
                } catch (e) {
                    logger.warn('Could not enter address, will skip instead', { error: e.message });
                    shouldEnterAddress = false; // Fallback to skip
                }
            }
            
            // If not entering address, click Skip or Cancel
            // إذا لم يتم إدخال العنوان، اضغط Skip أو Cancel
            if (!shouldEnterAddress || !addressEntered) {
                logger.info('Will skip address entry on home address page');
                
                let skipClicked = false;
                
                // Try to find Skip button
                try {
                    const skipButton = await page.$('button[aria-label="Skip"], button[jsname="ZUkOIc"]');
                    if (skipButton) {
                        const isVisible = await skipButton.evaluate(el => el.offsetParent !== null);
                        const isDisabled = await skipButton.evaluate(el => el.disabled);
                        if (isVisible && !isDisabled) {
                            await humanClick(page, skipButton, { preClickDelay: true, postClickDelay: true });
                            logger.info('Skip button clicked on home address page');
                            skipClicked = true;
                            await humanSleep(getHumanDelay('pageLoad'));
                        }
                    }
                } catch (e) {
                    logger.debug('Could not find Skip button', { error: e.message });
                }
                
                // If Skip not found, try Cancel
                if (!skipClicked) {
                    try {
                        ...
                        if (cancelButton) {
                            ...
                            if (isVisible && !isDisabled) {
                                await humanClick(page, cancelButton, { preClickDelay: true, postClickDelay: true });
                                logger.info('Cancel button clicked on home address page');
                                await humanSleep(getHumanDelay('pageLoad'));
                            }
                        }
                    } catch (e) {
                        logger.debug('Could not find Cancel button', { error: e.message });
                    }
                }
            }
            
            // Get new URL after handling
            try {
                currentUrl = page.url();
            } catch (e) {
                logger.debug('Could not get URL after home address page handling');
            }
        }

        // Step 6: Handle confirmidentifier page (after terms of service or at any point)
        // الخطوة 6: التعامل مع صفحة confirmidentifier (بعد شروط الخدمة أو في أي وقت)
        if (currentUrl.includes('confirmidentifier')) {
            logger.info('Confirm identifier page detected - clicking Next button');
            
            // Human behavior: Wait before clicking
            // سلوك بشري: الانتظار قبل النقر
            await humanSleep(getHumanDelay('thinking'));
            
            // Try multiple methods to find and click Next button
            // تجربة عدة طرق للعثور على زر Next والنقر عليه
            let nextClicked = false;
            
            // Method 1: Try XPath (from user specification)
            // الطريقة 1: تجربة XPath (من مواصفات المستخدم)
            ...
            try {
                ...
                if (confirmButtons.length > 0) {
                    ...
                    if (isVisible && !isDisabled) {
                            await humanClick(page, confirmButtons[0], { preClickDelay: true, postClickDelay: true });
                            logger.info('Next button clicked on confirmidentifier page (XPath)');
                            nextClicked = true;
                            await humanSleep(getHumanDelay('pageLoad'));
                        }
                    } catch (evalError) {
                        // If evaluation fails, try clicking directly
                        // إذا فشل التقييم، جرب النقر مباشرة
                        logger.debug('Evaluation failed, trying direct click', { error: evalError.message });
                        try {
                            await humanClick(page, confirmButtons[0], { preClickDelay: true, postClickDelay: true });
                            logger.info('Next button clicked on confirmidentifier page (direct click)');
                            nextClicked = true;
                            await humanSleep(getHumanDelay('pageLoad'));
                        } catch (clickError) {
                            logger.debug('Direct click also failed', { error: clickError.message });
                        }
                    }
                }
            } catch (e) {
                logger.debug('XPath method failed, trying other methods', { error: e.message });
            }
            
            // Method 2: Try clickNextButton function
            // الطريقة 2: تجربة دالة clickNextButton
            if (!nextClicked) {
                try {
                    const clicked = await clickNextButton(page);
                    if (clicked) {
                    ...
                    logger.debug('clickNextButton method failed', { error: fallbackError.message });
                }
            }
            
            // Method 3: Try finding button by text content
            // الطريقة 3: محاولة العثور على الزر من خلال محتوى النص
            if (!nextClicked) {
                try {
            const nextButton = await page.evaluateHandle(() => {
                const buttons = Array.from(document.querySelectorAll('button'));
            ...
            if (nextButton && nextButton.asElement()) {
                    ...
                    logger.debug('Text search method failed', { error: textSearchError.message });
                }
            }
            
            // Method 4: Try CSS selector for Next button
            // الطريقة 4: تجربة محدد CSS لزر Next
            if (!nextClicked) {
                try {
                    ...
                    for (const selector of nextButtonSelectors) {
                        try {
                            ...
                            if (button) {
                                ...
                                if (isVisible && !isDisabled) {
                                    await humanClick(page, button, { preClickDelay: true, postClickDelay: true });
                                    logger.info('Next button clicked via CSS selector', { selector });
                                    nextClicked = true;
                                    await humanSleep(getHumanDelay('pageLoad'));
                                    break;
                                }
            ...
            // انتظار التنقل بعد النقر
            if (nextClicked) {
                await humanSleep(getHumanDelay('pageLoad'));
            }
            
            // Get new URL (with error handling for navigation)
            // الحصول على URL جديد (مع معالجة الأخطاء للتنقل)
            try {
                currentUrl = page.url();
                if (nextClicked) {
                    logger.info('Navigation completed after clicking Next on confirmidentifier page', { newUrl: currentUrl });
                }
            } catch (urlError) {
                logger.debug('Could not get URL after confirmidentifier click (navigation may be in progress)', { 
                    error: urlError.message 
                });
                // Wait a bit more for navigation to complete
                // انتظار قليلاً لإكمال التنقل
                await humanSleep(getHumanDelay('pageLoad'));
                try {
                    currentUrl = page.url();
                } catch (e) {
                    // If still fails, continue with previous URL
                    // إذا استمر الفشل، استمر مع URL السابق
                    logger.warn('Could not get URL after waiting, continuing...');
                }
            }
            
            if (!nextClicked) {
                logger.warn('Could not click Next button on confirmidentifier page, but continuing...');
            }
        }

        // Click through any remaining steps (fallback) - with better error handling
        // النقر عبر أي خطوات متبقية (بديل) - مع معالجة أفضل للأخطاء
        for (let i = 0; i < 3; i++) {
            try {
                // Use safer method that doesn't rely on evaluateHandle
                // استخدام طريقة أكثر أماناً لا تعتمد على evaluateHandle
                const nextButtonXPath = '/html/body/div[2]/div[1]/div[1]/div[2]/c-wiz/main/div[3]/div/div/div/div/button';
                try {
                    const nextButtons = await page.$x(nextButtonXPath);
                    if (nextButtons.length > 0) {
                        try {
                            ...
                            // If evaluation fails, try clicking anyway
                            try {
                                await humanClick(page, nextButtons[0], { preClickDelay: true, postClickDelay: true });
                                await humanSleep(getHumanDelay('shortPause'));
                                continue;
                            } catch (clickError) {
                                // Navigation might have happened
                                break;
                            }
                        }
                    }
                } catch (xpathError) {
                    // Try clickNextButton as fallback
                    try {
                        const clicked = await clickNextButton(page);
                        if (clicked) {
                            await humanSleep(getHumanDelay('shortPause'));
                        ...
                        // Navigation might have happened or page changed
                        break;
                    }
                }
                ...
                // If execution context was destroyed, navigation likely happened
                // إذا تم تدمير execution context، فمن المحتمل أن التنقل حدث
                if (error.message.includes('Execution context was destroyed') || 
                    error.message.includes('navigation')) {
                    logger.debug('Execution context destroyed, navigation likely occurred', { iteration: i + 1 });
                    break;
                }
                logger.debug('Error in fallback click loop', { error: error.message, iteration: i + 1 });
                break;
            }
        }
