...
export async function detectGmailPage(page) {
    try {
        ...
        // Check for confirm identifier page (after terms of service)
        // التحقق من صفحة تأكيد الهوية (بعد شروط الخدمة)
        if (url.includes('/signin/confirmidentifier') || url.includes('/v3/signin/confirmidentifier')) {
            logger.info('Detected: Confirm Identifier page (by URL)', { url });
            return {
                type: 'CONFIRM_IDENTIFIER_AFTER_TERMS',
        ...
        // Check for recovery options page
        // التحقق من صفحة خيارات الاسترداد
        if (url.includes('/web/recoveryoptions') || url.includes('recoveryoptions')) {
            logger.info('Detected: Recovery Options page (by URL)', { url });
            return {
        ...
        // Check for home address page
        // التحقق من صفحة العنوان المنزلي
        if (url.includes('/web/homeaddress') || url.includes('homeaddress')) {
            logger.info('Detected: Home Address page (by URL)', { url });
            return {
                type: 'HOME_ADDRESS',
                elements: ['address_input', 'skip_button', 'save_button'],
                action: 'handle_address',
                url: url
            };
        }
